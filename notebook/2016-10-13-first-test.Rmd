...
try_parms = list(
  ...
  sigma = 0, # 1e5,                      #virion shedding rate
...
initial_cond <- matrix(c(595, 5, 0, 0),
...
options(mc.cores=2)
Rprof("out.prof")
output1 <- mf_sim(init = initial_cond, parameters = try_parms, times=0:1000, n_sims = 1000,  parallel = FALSE, .test=TRUE)
Rprof(NULL)
...
Rprof("out.prof")
aa = mclapply(1:1000, function(x) { sim_gillespie(initial_cond, try_parms, 1:1000, FALSE)}, mc.cores=1)
Rprof(NULL)
outputs = output1 %>% 
  filter(class == "I") %>% 
  group_by(sim, time) %>% 
