...
library(knitr)
opts_chunk$set(cache.path='~/.vignette_cache/', echo = FALSE, message=FALSE, warning = FALSE, autodep=TRUE)
...
proportions <- unlist(mclapply(fig2results, function(x) proportion_failed(get_failure(x))))
...
duration_list <- mclapply(fig2results, get_duration)
...
abundance_list <- mclapply(fig2results, get_tot_infections)
...
infectious_list <- mclapply(fig2results, get_infections)
recovered_list <- mclapply(fig2results, get_recovered)
susceptible_list <- mclapply(fig2results, get_susceptibles)
...
fig8parms = list(
  ...
  phi = 0,  #infectiousness of environmental virions
  eta = 0,     #degradation rate of environmental virions
  nu =  0.0,    #uptake rate of environmental virion
  sigma = 0,      #virion shedding rate
  omega = 0.03,      #movement rate
  rho = 0,        #contact  nonlinearity 0=dens-dependent, 1=freq-dependent
  lambda = 0,    #force of infection from external sources
