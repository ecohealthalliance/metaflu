...
initial_cond <- matrix(c(99, 1, 0, 0), nrow=64, ncol=4, byrow=TRUE)
...
paperparms = list(
  ...
  rho = 0,        #contact  nonlinearity 0=dens-dependent, 1=freq-dependent
  lambda = 0,    #force of infection from external sources
  chi = matrix(c(1,0,0,1), nrow=2),
  network_type = "smallworld",
  network_parms = list(dim = 6, size = 2, nei = 1, p = 0.1)
)
...
output1 <- mf_sim(init = initial_cond, parameters = parms, times=0:1000, n_sims = 1)
