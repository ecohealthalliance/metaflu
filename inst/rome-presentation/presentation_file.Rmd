...
create_gif(basic_results_list[[high_duration_sim]], P("inst","rome-presentation","base_high_dur_sim"), "Long Duration Simulation")
create_gif(basic_results_list[[low_duration_sim]], P("inst","rome-presentation","base_low_dur_sim"), "Short Duration Simulation")

```

![](`r P("inst","rome-presentation","base_high_dur_sim.gif")`)
...
![](`r P("inst","rome-presentation","base_low_dur_sim.gif")`) 
...
rg_viz <- random_growth_list[[1]]
start <- rg_viz[,,1,1]

rg_net <- attr(rg_viz, "network")[[1]]

rg_graph <- graph_from_adjacency_matrix(rg_net > 0, "undirected")

rg_layout <- layout_nicely(rg_graph,2) %>% as.data.frame() %>% rename(x=V1, y=V2)

sim_df <- plyr::adply(start, c(1,2)) %>%
    dplyr::rename(compartment = X1, patch = X2, population = V1) %>%
    tidyr::spread(compartment, population)

sim_df <- sim_df %>%
    mutate(x = as.numeric(rg_layout$x), y = as.numeric(rg_layout$y))

size_range <- c(4,11)

sim_df$growth <- if_else(sim_df$S >= min(sim_df$S)*10, "growth", "normal")

cols <- c("growth" = "chartreuse3", "normal" = "grey10")

rg_plot <- ggraph(rg_graph, 'manual', node.positions=rg_layout) +
    geom_edge_arc(curvature = 0.1, edge_colour='grey40') +
    geom_node_point(data=sim_df,
                    mapping=aes(x=x, y=y, fill= growth, size = S),
                    shape=21, stroke=1, color="grey10") +
    scale_fill_manual(values = cols) +
    scale_alpha_continuous(range=c(0.5,1)) +
    scale_size_continuous(range=size_range) +
    ggforce::theme_no_axes() +
  labs(title = "Random Growth Network") +
...
lg_plot <- ggraph(rg_graph, 'manual', node.positions=rg_layout) +
    geom_edge_arc(curvature = 0.1, edge_colour='grey40') +
    geom_node_point(data=sim_df,
                    mapping=aes(x=x, y=y, fill= growth, size = S),
                    shape=21, stroke=1, color="grey10") +
    ...
    scale_size_continuous(range=size_range) +
    ggforce::theme_no_axes() +
    labs(title = "Localized Growth Network") +
    theme(legend.position="none",
          panel.border = element_blank())
