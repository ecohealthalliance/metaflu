...
knitr::opts_chunk$set(echo = FALSE, cache = TRUE, fig.width=10, fig.height=6, warning = FALSE)
devtools::load_all()
library(metaflu)
library(ggplot2)
library(dplyr)
...
This document generates a guaranteed outbreak scenario, with __100 farms__ of __200 chickens__ each. The current simulation includes non-linear contact rate implemented via $\rho$ and $\beta$, along with clustered intensification: 9% of farms whose population increases are clustered together. There is no environmental transmission, background mortality rate, or surveillance / culling modeled in these simulations.
...
g_duration <- ggplot() +
  geom_histogram(data = g_durations, aes(x = durations$duration), binwidth = 0.5) +
  theme_minimal() +
  labs(title = paste0("Duration of Epidemic in ", l, " Simulations"), x = "Duration in Days", y = "Number of Simulations")
