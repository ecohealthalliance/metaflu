...
Approximately 9% of the original farms grow to ten-times their original size (~15 to ~150), and these farms are all in one neighborhood in this experiment. 

```{r growth-run, cache = TRUE}
#Grow an adjacent set of patches to get double total chicken population
set.seed(123)

s_num <- round(nrow(initial_cond)*1/11)
new_cond <- initial_cond
walk(seq_len(s_num), function(row){
    new_cond[row,1] <- new_cond[row,1]*10
  })
grown_cond <- new_cond
