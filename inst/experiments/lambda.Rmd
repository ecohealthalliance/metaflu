...
This document generates a guaranteed outbreak scenario, with __100 farms__ of __200 chickens__ each. The current simulation includes non-linear contact rate implemented via $\rho$ and $\beta$, along with clustered intensification: 11% of farms whose population increases are clustered together. There is no environmental transmission, background mortality rate, or surveillance / culling modeled in these simulations.
...
$\lambda$ = 6.849315e-07 -- External force of infection
...
```{r parameter-setup}
  parms = list(
    ...
    lambda = 6.849315e-07,     #force of infection from external sources
...
```{r basic-run}
basic_results <- mf_sim(init = initial_cond, parameters = parms, times=1:1825, n_sims = 1000)
