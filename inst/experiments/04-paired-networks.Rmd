...
```{r basic-run}
basic_results <- mf_sim(init = create_initial_condition(basic_patches(40,100)), parameters = parms, times=1:365, n_sims = 1000)
...
grow_patches <- function(old_condition){
...
gp <- cbind(gp, cumsum(gp[,1]))

rando <- sample.int(dim(gp)[1],1)

min(which(gp[,5] >= rando))

sims <- 1000
g_list <- map3(seq_len(sims),nets,inits, function(num, net, init){
  g_parms <- parms
  g_parms["network_type"] <- NULL
  g_parms["network_parms"] <- NULL
  g_parms["stochastic_network"] = FALSE
  g_parms$chi <- net
  return(mf_sim(init = init, parameters = g_parms, times=1:365, n_sims = 1))
} )
