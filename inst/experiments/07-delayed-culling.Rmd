...
set.seed(123)
conditions <- create_initial_condition(basic_patches(farm_size, farm_number))
res1 <- mf_sim(init = conditions, parameters = parms, times=1:365, n_sims = 1)
...
parms["cull_time"] <- 3
conditions <- create_initial_condition(basic_patches(farm_size, farm_number))
res1 <- mf_sim(init = conditions, parameters = parms, times=1:365, n_sims = 100)

culls <- res1["C",,,]

list_culls <- lapply(seq_len(dim(culls)[3]), function(x) rowSums(culls[,,x]))

non_zeros <- culls[which(list_culls[[1]] > 1),,1]

sbs <- function(results){
  rbind(res1["I",,,1][8,],res1["R",,,1][8,], res1["C",,,1][8,])
}
