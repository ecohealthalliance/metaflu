...
```{r basic-run}
basic_results <- mf_sim(init = initial_cond, parameters = parms, times=1:1825, n_sims = 10)
...
sir.df <- ggplot() +
  geom_line(data = s.df, aes(x = new_time, y = pop, group = sim), alpha = 0.5, color = "blue") +
  geom_line(data = i.df, aes(x = new_time, y = pop, group = sim), alpha = 0.5, color = "red") +
  geom_line(data = r.df, aes(x = new_time, y = pop, group = sim), alpha = 0.5, color = "green") +
